...
Vagrant.configure(2) do |config|
  ...
  config.vm.define :controlplane do |controlplane|
    ...
    controlplane.vm.synced_folder "./vendor", "/var/sync/vendor"

    ### This allows the node to default to the right IP i think....
    # 1) this seems to break the ability to get to the internet

    controlplane.vm.provision :shell, privileged: false, path: "sync/linux/01-controlplane.sh", args: "#{cfg_kubernetes_version} #{cfg_linux_node_ip} #{cfg_pod_cidr}"

    # TODO shoudl we pass KuberneteVersion to calico agent exe? and also service cidr if needed?
    # Do not run as priveliged as we need the kubeconfig from regular user
    controlplane.vm.provision "shell", path: "sync/linux/02-calico.sh", args: "#{cfg_pod_cidr} #{cfg_calico_version}"
  end
  ...
  config.vm.define :winw1 do |winw1|
    ...
    winw1.vm.synced_folder "./vendor", "C:/vendor/"
    ...
    if not File.file?(".lock/joined") then
      # Update containerd
      winw1.vm.provision "shell", path: "sync/windows/01-containerd.ps1", args: "#{cfg_calico_version} #{cfg_containerd_version}", privileged: true
      # Joining the controlplane
      #winw1.vm.provision "shell", path: "sync/windows/02-kubernetes.ps1", args: "#{cfg_kubernetes_version}", privileged: true
      # Create mock file to make Vagrantfile happy. Runnable sync/shared/kubejoin.ps1 is generated by 01-controlplane.sh
      FileUtils.touch("sync/shared/kubejoin.ps1") unless File.exist?("sync/shared/kubejoin.ps1")
      #TODO winw1.vm.provision "shell", path: "sync/shared/kubejoin.ps1", privileged: true #, run: "never"
    else
