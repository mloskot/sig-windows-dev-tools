...
'
set -e
ME="01-controlplane.sh" # $(basename "$0") reports vagrant-shell instead of script name
function echolog
{
...
if [[ "$1" == "" || "$2" == "" || "$3" == "" ]]; then
  cat << EOF
    ...
    ${ME} 1.21 10.20.30.10 100.244.0.0/16
...
echolog "Using ${kubernetes_version} as the Kubernetes version"

# Installing packages

# Add GPG keys and repository for Kubernetes
echolog "Setting up internet connectivity to /etc/resolv.conf"
cat <<EOF | sudo tee /etc/resolv.conf
nameserver 8.8.8.8
nameserver 1.1.1.1
EOF

echolog "Adding Kubernetes APT keys"
curl -s https://packages.cloud.google.com/apt/doc/apt-key.gpg | sudo apt-key add -
cat << EOF | sudo tee /etc/apt/sources.list.d/kubernetes.list
deb https://apt.kubernetes.io/ kubernetes-xenial main
EOF

echolog "Running APT update"
sudo apt-get update -y

echolog "Disabling swap"
sudo swapoff -a
...
echolog "Running modprobe"
sudo modprobe overlay
sudo modprobe br_netfilter

echolog "Seting up required sysctl params, these persist across reboots"
...
echolog "Applying sysctl params without reboot"
sudo sysctl --system

# Install Kubernetes binaries, using latest available, and
# overwritting the binaries later.

echolog "Installing kubelet, kubeadm, kubectl will overwrite them later as needeed" 
sudo apt-get install -y kubelet kubeadm kubectl
sudo apt-mark hold kubelet kubeadm kubectl

# Install containerd - latest version available
echolog "Installing containerd"
...
echolog "Checking containerd version"
containerd --version

echolog "Overwriting Kubernetes binaries with local ones"
...
echolog "Configuring and starting containerd"
...
echolog "Preparing to start the cluster with the current user $USER"
mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config

echolog "Copying $HOME/.kube/config to sync/shared/kubeconfig for Windows node"
rm -f /var/sync/shared/kubeconfig
cp $HOME/.kube/config /var/sync/shared/kubeconfig

echolog "Creating join file for Windows node in /var/sync/shared/kubejoin.ps1"
...
echolog "Creating kube-proxy.yaml with ServiceAccount kube-proxy-windows"
...
echolog "Listing pods on control plane node"
kubectl get pods -A
